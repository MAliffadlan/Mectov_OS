...
/* Mectov (v38.98): kernel heap is bounded; small hunk/zone by default. */
...
static void Q3Z_CheckHeap( void );
...
========================
*/
static void Q3Z_ClearZone( memzone_t *zone, int size ) {
	memblock_t	*block;
...
/*
========================
Q3Z_AvailableZoneMemory
========================
*/
static int Q3Z_AvailableZoneMemory( memzone_t *zone ) {
...
========================
*/
int Q3Z_AvailableMemory( void ) {
	return Q3Z_AvailableZoneMemory( mainzone );
}
...
========================
*/
void Q3Z_Free( void *ptr ) {
	memblock_t	*block, *other;
	memzone_t *zone;
	
	if (!ptr) {
		Com_Error( ERR_DROP, "Q3Z_Free: NULL pointer" );
	}

	block = (memblock_t *) ( (byte *)ptr - sizeof(memblock_t));
	if (block->id != ZONEID) {
		Com_Error( ERR_FATAL, "Q3Z_Free: freed a pointer without ZONEID" );
	}
	if (block->tag == 0) {
		Com_Error( ERR_FATAL, "Q3Z_Free: freed a freed pointer" );
	}
	// if static memory
	...
	if ( *(int *)((byte *)block + block->size - 4 ) != ZONEID ) {
		Com_Error( ERR_FATAL, "Q3Z_Free: memory block wrote past end" );
	}
...
void Q3Z_FreeTags( int tag ) {
	...
	// Q3Z_Free automatically adjusts it
	zone->rover = zone->blocklist.next;
	do {
		if ( zone->rover->tag == tag ) {
			Q3Z_Free( (void *)(zone->rover + 1) );
			continue;
		}
...
*/
#ifdef ZONE_DEBUG
void *Q3Z_TagMallocDebug( int size, int tag, char *label, char *file, int line ) {
	int		allocSize;
#else
void *Q3Z_TagMalloc( int size, int tag ) {
#endif
	int		extra;
	...
	if (!tag) {
		Com_Error( ERR_FATAL, "Q3Z_TagMalloc: tried to use a 0 tag" );
	}
	...
	do {
		if (rover == start)	{
			...
			Com_Error(ERR_FATAL, "Q3Z_Malloc: failed on allocation of %i bytes from the %s zone: %s, line: %d (%s)",
								size, zone == smallzone ? "small" : "main", file, line, label);
#else
			Com_Error(ERR_FATAL, "Q3Z_Malloc: failed on allocation of %i bytes from the %s zone",
...
*/
#ifdef ZONE_DEBUG
void *Q3Z_MallocDebug( int size, char *label, char *file, int line ) {
#else
void *Q3Z_Malloc( int size ) {
#endif
	void	*buf;
...
#ifdef ZONE_DEBUG
	buf = Q3Z_TagMallocDebug( size, TAG_GENERAL, label, file, line );
#else
	buf = Q3Z_TagMalloc( size, TAG_GENERAL );
#endif
...
void *S_MallocDebug( int size, char *label, char *file, int line ) {
	return Q3Z_TagMallocDebug( size, TAG_SMALL, label, file, line );
}
#else
void *S_Malloc( int size ) {
	return Q3Z_TagMalloc( size, TAG_SMALL );
}
#endif
...
static void Q3Z_CheckHeap( void ) {
	...
	for (block = mainzone->blocklist.next ; ; block = block->next) {
		...
		if ( (byte *)block + block->size != (byte *)block->next)
			Com_Error( ERR_FATAL, "Q3Z_CheckHeap: block size does not touch the next block" );
		if ( block->next->prev != block) {
			Com_Error( ERR_FATAL, "Q3Z_CheckHeap: next block doesn't have proper back link" );
		}
		if ( !block->tag && !block->next->tag ) {
			Com_Error( ERR_FATAL, "Q3Z_CheckHeap: two consecutive free blocks" );
		}
	}
}
...
========================
*/
void Q3Z_LogZoneHeap( memzone_t *zone, char *name ) {
#ifdef ZONE_DEBUG
	char dump[32], *ptr;
	int  i, j;
#endif
...
========================
*/
void Q3Z_LogHeap( void ) {
	Q3Z_LogZoneHeap( mainzone, "MAIN" );
	Q3Z_LogZoneHeap( smallzone, "SMALL" );
}
...
void Com_InitSmallZoneMemory( void ) {
	...
	Q3Z_ClearZone( smallzone, s_smallZoneTotal );
}
...
void Com_InitZoneMemory( void ) {
	...
	Q3Z_ClearZone( mainzone, s_zoneTotal );
...
#ifdef ZONE_DEBUG
	Cmd_AddCommand( "zonelog", Q3Z_LogHeap );
#endif
#ifdef HUNK_DEBUG
...
void *Hunk_AllocateTempMemory( int size ) {
	...
	// return a Q3Z_Malloc'd block if the hunk has not been initialized
	// this allows the config and product id files ( journal files too ) to be loaded
	// by the file system without redunant routines in the file system utilizing different 
	// memory systems
	if ( s_hunkData == NULL )
	{
		return Q3Z_Malloc(size);
	}
...
void Hunk_FreeTempMemory( void *buf ) {
	hunkHeader_t	*hdr;
	  ...
	  // free with Q3Z_Free if the hunk has not been initialized
	  // this allows the config and product id files ( journal files too ) to be loaded
	  // by the file system without redunant routines in the file system utilizing different 
	  // memory systems
	if ( s_hunkData == NULL )
...
sysEvent_t	Com_GetRealEvent( void ) {
	...
	if ( com_journal->integer == 2 ) {
		...
		if ( ev.evPtrLength ) {
			ev.evPtr = Q3Z_Malloc( ev.evPtrLength );
