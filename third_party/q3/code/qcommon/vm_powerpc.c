 ...
 * Q3Z_Malloc memory may not be enough for some big qvm files
 */
...
#else
# define PPC_Malloc Q3Z_Malloc
# define PPC_Free Q3Z_Free
#endif
...
VM_Compile( vm_t *vm, vmHeader_t *header )
{
	...
	// use Q3Z_Malloc so vm.c will be able to free the memory
	if ( sizeof( void * ) != sizeof( int ) ) {
		Q3Z_Free( vm->instructionPointers );
		vm->instructionPointers = Q3Z_Malloc( header->instructionCount * sizeof( void * ) );
	}
