...
static unsigned int sparc_push_data(struct func_info * const fp, unsigned int val)
{
	...
	if (!dp || dp->count >= HUNK_SIZE) {
		struct data_hunk *new = Q3Z_Malloc(sizeof(*new));
		if (!dp)
			fp->data_first = new;
		else
			dp->next = new;
...
{
	struct dst_insn *dp = Q3Z_Malloc(sizeof(struct dst_insn) + insns_size);
...
{
	struct jump_insn *jp = Q3Z_Malloc(sizeof(*jp));
	struct dst_insn *dp;
...
void VM_Compile(vm_t *vm, vmHeader_t *header)
{
	...
	fi.first = Q3Z_Malloc(sizeof(struct src_insn));
	fi.first->next = NULL;

#ifdef __arch64__
	Q3Z_Free(vm->instructionPointers);
	vm->instructionPointers = Q3Z_Malloc(header->instructionCount *
					   sizeof(void *));
#endif
