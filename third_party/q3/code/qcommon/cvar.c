...
//	Com_DPrintf( "Cvar_Set2: %s %s\n", var_name, value );
	...
	Q3Z_Free (var->string);	// free the old value string
...
cvar_t *Cvar_Unset(cvar_t *cv)
{
	...
	if(cv->string)
		Q3Z_Free(cv->string);
	if(cv->latchedString)
		Q3Z_Free(cv->latchedString);
	if(cv->resetString)
		Q3Z_Free(cv->resetString);
	if(cv->description)
		Q3Z_Free(cv->description);
