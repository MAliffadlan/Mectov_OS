...
static pack_t *FS_LoadZipFile(const char *zipfile, const char *basename)
{
	...
	buildBuffer = Q3Z_Malloc( (gi.number_entry * sizeof( fileInPack_t )) + len );
	namePtr = ((char *) buildBuffer) + gi.number_entry * sizeof( fileInPack_t );
	fs_headerLongs = Q3Z_Malloc( ( gi.number_entry + 1 ) * sizeof(int) );
	...
	pack = Q3Z_Malloc( sizeof( pack_t ) + i * sizeof(fileInPack_t *) );
	pack->hashSize = i;
...
char **FS_ListFilteredFiles( const char *path, const char *extension,
	...
	*numfiles = nfiles;
	...
	listCopy = Q3Z_Malloc( ( nfiles + 1 ) * sizeof( *listCopy ) );
	for ( i = 0 ; i < nfiles ; i++ ) {
		listCopy[i] = list[i];
	}
	listCopy[i] = NULL;
...
static char** Sys_ConcatenateFileLists( char **list0, char **list1 )
{
	...
	/* Create new list. */
	dst = cat = Q3Z_Malloc( ( totalLength + 1 ) * sizeof( char* ) );
	...
	*dst = NULL;
	...
	if (list0) Q3Z_Free( list0 );
	if (list1) Q3Z_Free( list1 );
...
void FS_SortFileList(char **filelist, int numfiles) {
	...
	sortedlist = Q3Z_Malloc( ( numfiles + 1 ) * sizeof( *sortedlist ) );
	sortedlist[0] = NULL;
	numsortedfiles = 0;
...
static void FS_AddGameDirectory( const char *path, const char *dir ) {
	...
	while((pakfilesi < numfiles) || (pakdirsi < numdirs))
	{
		...
		if (pakwhich) {
			...
			search = Q3Z_Malloc(sizeof(searchpath_t));
			search->pack = pak;
			...
			search = Q3Z_Malloc(sizeof(searchpath_t));
			search->dir = Q3Z_Malloc(sizeof(*search->dir));
...
static void FS_CheckPak0( void )
{
	...
	/* Mectov (v38.98): a from-scratch OS has no id pak files and no CD-ROM;
	 * the retail pak legitimacy check cannot apply. Game data will be
	 * mounted in a later phase (mectov pak over miniz). */
	...
	/* Mectov (v38.98): com_standalone is CVAR_ROM so the Cvar_Set above is
	 * ignored; gate the retail-pak requirement out here instead. A
	 * from-scratch OS has no id paks — baseq3 data arrives in a later phase. */
	if(0 && !com_standalone->integer && (foundPak & ((1<<NUM_ID_PAKS)-1)) != ((1<<NUM_ID_PAKS)-1))
	{
...
void FS_PureServerSetReferencedPaks( const char *pakSums, const char *pakNames ) {
	...
	for (i = 0 ; i < ARRAY_LEN(fs_serverReferencedPakNames); i++)
	{
		if(fs_serverReferencedPakNames[i])
			Q3Z_Free(fs_serverReferencedPakNames[i]);
