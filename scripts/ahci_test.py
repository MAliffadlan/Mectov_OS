#!/usr/bin/env python3
"""
scripts/ahci_test.py — regression for the AHCI/SATA driver (v38.50).

Boots mectov.iso in QEMU with an ich9-ahci controller and a FAT32 SATA disk
(drive 4), then verifies a full READ+WRITE round trip through the new
driver, end to end, using the runtime mount table:

  1. the controller came up                    ("[AHCI] controller @"
     + "port ... -> drive 4" + "[AHCI] ready")
  2. `mount /sata fat32 4` mounts it           ("[MOUNT] mounted /sata") —
     reading the FAT32 BPB through AHCI is the read-path proof
  3. `cp /sata/HELLO.TXT /sata/COPY.TXT` — data read + dirent/data write
  4. the HOST reads COPY.TXT back with mtools and byte-compares it — the
     write-path proof (data actually landed on the SATA disk)
  5. the OS stayed alive afterwards

Usage:
    python3 scripts/ahci_test.py [--timeout 240]
"""
import argparse
import os
import socket
...
SERIAL_LOG = "/tmp/mectov_ahci_serial.log"
MON_SOCK = "/tmp/mectov_ahci_monitor.sock"
CONTENT = b"Hello from a SATA disk via AHCI!\n"

LOGIN_KEYS = ["spc", "m", "e", "c", "t", "o", "v", "1", "2", "3", "ret"]

KEYS_MOUNT = list("mount /sata fat32 4") + ["ret"]
KEYS_CP = list("cp /sata/HELLO.TXT /sata/COPY.TXT") + ["ret"]
...
def ensure_sata_image(path):
    hello = os.path.join(tempfile.gettempdir(), "ahci_hello.txt")
...
def ensure_boot_images(disk, ext2):
    # Mirror CI's "Create disk images" step. The runtime mount point /sata
...
def main():
    ...
    ap.add_argument("--sata", default="sata.img")
    args = ap.parse_args()

    if ensure_boot_images(args.disk, args.ext2) != 0:
        return 1
    if ensure_sata_image(args.sata) != 0:
        return 1
    print("[OK] SATA FAT32 image ready")
    ...
    qemu_cmd = [
        ...
        "-net", "none",
        # IDE fleet (drives 0-3) + one SATA disk behind ich9-ahci (drive 4)
        "-drive", f"file={args.disk},format=raw,index=0,media=disk",
        "-drive", f"file={args.ext2},format=raw,index=1,media=disk",
        "-device", "ich9-ahci,id=sata0",
        "-drive", f"file={args.sata},format=raw,if=none,id=satad0",
        "-device", "ide-hd,drive=satad0,bus=sata0.0",
    ...
    try:
        ...
        if not wait_for_in_file(SERIAL_LOG, "[AHCI] controller @", 15):
            print("[FAIL] AHCI controller not detected")
            return 1
        if not wait_for_in_file(SERIAL_LOG, "-> drive 0x00000004", 15) or \
           not wait_for_in_file(SERIAL_LOG, "[AHCI] ready", 15):
            print("[FAIL] SATA disk did not register as drive 4")
            return 1
        print("[OK] AHCI controller + SATA disk as drive 4")
        ...
        # Mount the SATA volume — fat32_init reads the BPB THROUGH AHCI.
        if not run_shell_cmd(KEYS_MOUNT, "[MOUNT] mounted /sata",
                             "mount /sata fat32 4 (BPB read via AHCI)"):
            return 1
        # Copy a file INSIDE the volume: read HELLO.TXT + write COPY.TXT,
        # both through the SATA driver. (cp prints to the GUI terminal, not
        ...
        if qemu.poll() is not None:
            ...
            return 1
        print("[OK] OS stayed alive after the SATA round trip")
    finally:
        qemu.kill()
        try:
    ...
    # Host-side proof: the bytes the guest wrote must be on the SATA image.
    out = os.path.join(tempfile.gettempdir(), "ahci_copy_out.txt")
    try:
        os.unlink(out)
    except FileNotFoundError:
        pass
    mc = subprocess.run(["mcopy", "-i", args.sata, "::COPY.TXT", out],
                        stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    if mc.returncode != 0:
        print("[FAIL] mtools cannot read COPY.TXT off the SATA image")
        return 1
    ...
    if data != CONTENT:
        ...
        return 1
    print("[OK] host reads back the exact bytes the guest wrote over SATA")
    return 0
