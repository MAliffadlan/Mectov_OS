# Mectov OS v36.7 — Per-CPU SMP Scheduler
...
The v36.7 release makes the kernel genuinely use all four CPU cores. Until now the "SMP" build only ticked the BSP: application processors woke, printed `[SMP] CPU n`, and then idled forever in `hlt` — every task, the GUI, and the shell ran on core 0, so the other three cores were decoration. v36.7 replaces the single global task list with **per-CPU runqueues**: each core owns its own queue of runnable tasks, the scheduler picks from its own queue with priority+aging, steals work from peer cores when idle (load-balancing migration), and parks in a pinned Ring 0 idle task per core instead of stealing the kernel main loop. Every Application Processor now gets a **LAPIC timer** (PIT-calibrated once on the BSP before the APs wake — concurrent per-AP calibration corrupted the shared PIT reading and caused timer storms that froze the system) so each core ticks at 1 kHz and drives its own scheduling. The result: `smpstress` forks 8 children that burn CPU, sleep, pipe, and signal each other across all four cores in two parallel waves — all 16 exit and are reaped correctly.

Getting here required a strict locking regime that the old single-core scheduler never needed. Every runqueue mutation (enqueue on fork/thread-create/wake, remove on sleep/block/stop/exit) happens under one `task_lock` with interrupts disabled first — so a timer IRQ can never self-deadlock in `schedule()`. The COW fork path, signal delivery on the syscall-return path, waitpid wakeups, and zombie reaping were all audited and re-locked. This hunt also fixed two real bugs: a **missing `spin_unlock` in `task_signal`** on the "handler installed" path leaked `task_lock` and froze the whole OS at the next `waitpid`, and `rq_least_loaded` was scanning all 16 CPU slots instead of the 4 real ones, parking new tasks on phantom cores that never tick. Serial output from four CPUs is now serialized per write (`write_serial_buffer`), so multi-core log lines no longer garble into each other, and the BSP alone owns the wall clock and GUI heartbeat while APs just schedule.

The v36.x line turns the kernel from a "launcher of apps" into a real Unix process model. v36.1 fixed the KVM-only **fork GPF**; v36.2 added **`exec()`**, System V **shared memory**, and the **trap-gate** syscall entry; v36.3 added **demand-paged `mmap()`** and **Ctrl+C**; v36.4 turned pipelines/redirection into real fork/exec/dup2 mechanisms, completed **job control** (Ctrl+Z/SIGSTOP/SIGCONT, real `bg`/`fg`), and added **TCP server** support; v36.5 hardened the **signal framework** into POSIX shape (sigaction/sa_mask/sigprocmask/SA_RESTART). The v36.6 release adds **process groups, sessions and a controlling terminal** — the last big piece of POSIX job control. New syscalls `SYS_SETPGID` (88), `SYS_GETPGRP` (89), `SYS_SETSID` (90), `SYS_TCSETPGRP` (91) and `SYS_TCGETPGRP` (92), backed by per-task `pgrp`/`session` fields that fork and thread creation inherit and exec preserves. The terminal becomes a session leader (`setsid`) and claims the foreground group (`tcsetpgrp`); the shell gives every launched app its own process group, marks it foreground while it runs, and hands the terminal back to the shell's group when it exits. **SIGTTIN** (default action: stop) is delivered when a background-group task reads the terminal — `bgread` forks, moves its child into its own background group with `setpgid(0,0)`, and the child is stopped mid-`read` instead of stealing keyboard input, then resumed with SIGCONT and reaped. Ctrl+C and Ctrl+Z now target the foreground **process group** (`task_signal_pgrp`) instead of walking the task tree, with the old tree walk kept as a fallback when no group is set.
...
| Version | Highlights |
|---|---|
| v36.6 | **Process Groups & Controlling Terminal (Fase 2) Update:** New `SYS_SETPGID` (88) / `SYS_GETPGRP` (89) / `SYS_SETSID` (90) / `SYS_TCSETPGRP` (91) / `SYS_TCGETPGRP` (92). Each task gains `pgrp` + `session` fields: init/kernel tasks start at 0; fork and thread_create inherit the caller's group; exec preserves it (POSIX). The terminal (`terminal.mct`) calls `setsid()` + `tcsetpgrp(1, getpid())` at startup to become session leader and claim the foreground group; the shell (`run`) gives each app its own `pgrp == tid`, sets the foreground group while the app runs, and `task_cleanup` hands the terminal back to the **shell's** group (the app's parent — not the app's own group, which would point at a corpse) on exit. `SYS_GET_KEY` delivers **SIGTTIN** (new signal 21, default action: stop) when a background-group task reads the controlling terminal, so a background job cannot steal keyboard input; SIGTTOU (22) is defined with the same default. Ctrl+C/Ctrl+Z in `kernel.c` now call `task_signal_pgrp(fg_pgrp)` (falling back to the old `task_signal_group` tree walk when no group is set), so a pipeline and its children stop/interrupt together. `bgread` demo: forks, the child `setpgid(0,0)`s into its own background group, reads the tty, gets stopped by SIGTTIN (verified via serial), is resumed by SIGCONT and reaped. Verified KVM (bgread, Ctrl+Z/bg/kill, sigdemo, pipe) and TCG (boot/fork/jobcontrol) all green. |
| v36.5 | **Signal Hardening — sigaction, sigprocmask, SA_RESTART Update:** New `SYS_SIGACTION` (86) and `SYS_SIGPROCMASK` (87). Each task gains `sig_masks[SIG_MAX]` (sa_mask), `sig_flags[SIG_MAX]` (SA_RESTART=1, SA_NODEFER=2) and a `blocked_signals` bitmap. `task_signal` defers blocked signals — it marks them pending but never wakes a parked task, and the delivery loop in `task_deliver_signals` skips them, so a signal cannot race ahead of its own unblock; SIGKILL/SIGSTOP/SIGCONT bypass the mask entirely. Handler delivery auto-blocks the delivered signal (unless SA_NODEFER) plus `sa_mask`, saving the prior mask in the (extended) sigframe; `SYS_SIGRETURN` restores it first. `SA_RESTART` handles SYS_SLEEP: when a SA_RESTART handler interrupts a sleep, `sig_restart_ticks` records the remaining ticks, delivery carries them in the sigframe, and sigreturn re-parks the task (`task_sleep(remaining)`) before iret — the restart attaches only to the delivered signal's own SA_RESTART flag, not to whatever pending signal happens to sort first. fork copies masks/actions; exec preserves the mask per POSIX while resetting caught handlers to default. `sigdemo` proves: blocked signal stays pending until unblocked, unblock delivers it, sa_mask defers SIGUSR2 inside a SIGUSR1 handler, and SA_RESTART keeps a 100-tick sleep at full duration across an interrupting handler. Verified KVM (sigdemo + pipe + stop + boot regressions) and TCG (boot/fork/jobcontrol) all green. |
| v36.4 | **Real Pipelines, App Redirection & TCP Server Update:** New `SYS_DUP2` (84) + per-task fd-table inheritance: `do_sys_dup2_tid` refcounts global fds POSIX-style. `task_fork_exec()` builds the child's image kernel-side first, allocates a fresh Ring 3 stack, inherits the caller's fd table, then `task_rewire_fds` dup2s stdin/stdout onto the pipe/file ends and closes every other descriptor — so the child is a brand-new process directly exec'd into the app (parent = shell, waitpid-able). Shell: `run A | run B` detects both sides and spawns them concurrently through a kernel pipe (`pipegen` writes 30 bytes, `piperead` reads them all and sees EOF when the writer exits — the rewrite loop originally left the pipe's write end referenced in the reader, so EOF never fired; fixed by closing **all** fds except the wired 0/1); `run A > file` redirects an external app's stdout (the fix also surfaced a latent fd-write bug: `do_sys_write` to a file always wrote from offset 0, so the second write clobbered the first — it now honors `global_fds[gfd].offset`). **Job control completed:** SIGTSTP on Ctrl+Z (scancode 0x2C), SIGSTOP/SIGCONT states in `task_signal()`, stopped jobs registered for the shell, `bg` sends SIGCONT for real, `jobs` shows Stopped, `fg` resumes and waits. **TCP server:** `net_tcp_listen(port)` reserves a slot in a new `TCP_LISTEN` state; `net_handle_tcp` walks it to SYN_RCVD on a matching SYN (sends SYN-ACK, ACKs the peer's ISN) and to ESTABLISHED on the final ACK — `SYS_TCP_LISTEN` (85) was added to the syscall table **and** routed to `handle_syscall_net` (the routing gap made it return -1 silently). `tcpserver` (echo app) verified end-to-end: host connects through QEMU hostfwd, sends a payload, gets it echoed back intact, then a clean CLOSE_WAIT → FIN → final ACK. Verified KVM + TCG: tcp, pipe, redir-app, stop (Ctrl+Z), exec, shm, mmap, ctrl+c, fork, boot, jobcontrol all green. |
