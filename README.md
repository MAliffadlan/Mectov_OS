...
- **Login gate (v38.30–v38.32)**: full-bleed scaled wallpaper (never cropped), digital clock + full date (day, date, month, year) pinned bottom-left, SPACE-only password entry, and a 4-second idle fallback back to the clock view. While the password panel is open (v38.31) the wallpaper **blurs** and the clock/date + version footer **disappear**, so the panel is the only thing in focus. Every switch between the two screens **cross-fades** (v38.32): the blur eases in/out, the captions fade, and the panel fades — one lerp pass animates all of it, in both directions.
