# Mectov OS v36.4 — Real Pipelines, App Redirection & TCP Server Update
...
The v36.x line turns the kernel from a "launcher of apps" into a real Unix process model. v36.1 fixed the KVM-only **fork GPF** (the fork frame now derives from the kernel-stack top, where the live `int 0x80` frame always sits); v36.2 added **`exec()`** (completing fork/exec/waitpid), System V **shared memory**, and the **trap-gate** syscall entry; v36.3 added **demand-paged `mmap()`** and **Ctrl+C** SIGINT job interruption. The v36.4 release turns the shell's pipelines and redirection into **real Unix mechanisms**: new `SYS_DUP2` (84) and per-task fd-table inheritance, plus `task_fork_exec()` — a single call that spawns a child **directly exec'd** into an app image (no wasted COW copy), with its stdin/stdout rewired to a kernel pipe or file and every other descriptor dropped. So `run /apps/pipegen.mct | run /apps/piperead.mct` streams bytes between two real concurrent processes and EOF propagates when the writer exits, and `run /apps/pipegen.mct > file` writes an external app's output to a file (the file write path now honors per-fd offsets, so multiple writes append instead of last-write-wins). Job control is now complete: **Ctrl+Z (SIGTSTP)** stops the foreground job, **`bg`** really resumes a stopped job via SIGCONT, **`jobs`** shows `Stopped` state, and **`fg`** resumes it. And the network stack can now be a **TCP server**: `net_tcp_listen()` + LISTEN/SYN_RCVD states complete the passive handshake (SYN → SYN-ACK → final ACK), exposed as `SYS_TCP_LISTEN` (85) — the `tcpserver` echo app is verified **end-to-end from the host machine** through QEMU hostfwd.
