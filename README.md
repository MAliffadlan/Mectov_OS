...
- **Login gate (v38.30–v38.31)**: full-bleed scaled wallpaper (never cropped), digital clock + full date (day, date, month, year) pinned bottom-left, SPACE-only password entry, and a 4-second idle fallback back to the clock view. While the password panel is open (v38.31) the wallpaper **blurs** and the clock/date + version footer **disappear**, so the panel is the only thing in focus.
...
| `scripts/login_redesign_test.py` | Redesigned login gate: wallpaper full-bleed + amber clock at bottom-left, non-SPACE key ignored, SPACE opens the password panel, wallpaper blurs + clock/footer hidden while typing, panel stays up, 4 s idle reverts to the clock view, password still logs in (TCG) |
