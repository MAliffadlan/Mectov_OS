...
- **Ownership, permissions & real threads (v38.23–v38.24, v38.27)** — every file carries a POSIX uid/gid + 9-bit mode enforced at the syscall layer (`chmod`/`chown`/`ls -l`, root bypass, ext2-persisted); every task has a private `%gs` TLS descriptor with `clone()` + a pthread-style runtime, so thread-local variables are genuinely per-thread; and futex-based `mct_mutex_t`/`mct_cond_t` (pthread-style) are proven on 4 cores by `conddemo`.
...
CI (`.github/workflows/build-boot-test.yml`) runs every suite above on each push: boot, fork, jobcontrol, KVM fork, FAT32, poll/select, fuzz, page cache, app smoke tests, mmap, Rust, lseek, panic, permissions, TLS/clone, futex/condvar, and multi-sector PIO.
