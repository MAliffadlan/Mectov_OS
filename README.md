...
- **Multi-sector PIO + `rep insw`/`rep outsw` (v38.25)**: the ATA driver issued one command per 512-byte sector and moved each sector word-by-word (256 port reads). Contiguous runs now go through a single command per up-to-16-sector batch (`ata_read_sectors_drive`/`ata_write_sectors_drive`, 128-boundary-safe), and each sector's data phase is ONE `rep insw`/`rep outsw`. Under KVM a 160 KB uncached read (the page cache stops at 128 KB) dropped **~680 ms → ~42 ms (~16×)**; the batching alone adds ~1.7× on top of `rep insw`, and TCG still gains ~11%.
...
| `scripts/bigread_test.py` | Multi-sector PIO + `rep insw`: 160 KB uncached file read 3× with byte-pattern verification on every read (a batch-offset or DRQ bug cannot pass silently); `--kvm` for real timing (TCG in CI) |
