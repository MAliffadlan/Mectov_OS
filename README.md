# Mectov OS v36.5 — Signal Hardening: sigaction, sigprocmask, SA_RESTART
...
The v36.x line turns the kernel from a "launcher of apps" into a real Unix process model. v36.1 fixed the KVM-only **fork GPF**; v36.2 added **`exec()`**, System V **shared memory**, and the **trap-gate** syscall entry; v36.3 added **demand-paged `mmap()`** and **Ctrl+C**; v36.4 turned pipelines/redirection into real fork/exec/dup2 mechanisms, completed **job control** (Ctrl+Z/SIGSTOP/SIGCONT, real `bg`/`fg`), and added **TCP server** support. The v36.5 release hardens the **signal framework** from a bare "set a handler" API into POSIX-shaped semantics: new `SYS_SIGACTION` (86) and `SYS_SIGPROCMASK` (87) give every signal a handler plus an `sa_mask` and flags, and each task carries a blocked-signal bitmap. Blocked signals are held pending and delivered only after `sigprocmask` unblocks them (never waking a parked task, so delivery cannot race ahead of the unblock; SIGKILL/SIGSTOP/SIGCONT are never blockable). While a handler runs, the delivered signal is auto-blocked (unless `SA_NODEFER`) together with `sa_mask`, and `SYS_SIGRETURN` restores the pre-handler mask — so a handler can no longer re-enter itself or clobber signals the app is holding. **`SA_RESTART`** preserves the remaining `SYS_SLEEP` duration across a handler: the interrupted sleep's leftover ticks ride in the sigframe and the task re-parks before returning to user mode. `fork` inherits the mask and actions; `exec` keeps the mask (POSIX) while resetting caught handlers to default. The `sigdemo` app verifies all five behaviors headless (blocked-held, deliver-on-unblock, sa_mask deferral, auto-block, SA_RESTART sleep preservation).
