// --- MECTOV OS v9.0 (Hardware Identity Update) ---
...
void detect_cpu() {
    ...
    // Cek apakah CPU mendukung Brand String (0x80000002 - 0x80000004)
...
const char* marquee_text = ">>> Mectov OS v9.0 - Hardware Detection Active - Indonesia Raya Project - Custom Kernel OK! <<<         ";
int marquee_pos = 0;
...
void ex_cmd() { if (!is_script) print("\n", 0x0F); cmd_b[b_idx] = '\0'; 
    if (strcmp(cmd_b, "matikan") == 0) shutdown();
        ...
        print("      /     \\       --------------\n", 0x0B);
        print("     | () () |      CPU: ", 0x0E); print(cpu_brand, 0x0A); print("\n", 0x0F);
        print("      \\  ^  /       OS : MectovOS v9.0\n", 0x0B);
...
void kernel_main(void) {
    ...
    print("Login Success! Hardware Identity Loaded.\nroot@mectov:~# ", 0x0A);
    while (1) {
        update_marquee();
        if (inb(0x64) & 1) {
            unsigned char sc = inb(0x60);
